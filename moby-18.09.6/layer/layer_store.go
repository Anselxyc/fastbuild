...
func (ls *layerStore) ConfusedLayer(parent ChainID, cacheID string) (Layer, error) {
	var err error
	var p *roLayer
	...
	layer.diffID = DiffID(digest.FromString(stringid.GenerateRandomID() + strconv.FormatInt(time.Now().UnixNano(), 10)))
	layer.size = 0
...
func (ls *layerStore) GetReferenceRWLayer(id string) (RWLayer, error) {
	ls.mountL.Lock()
	mount := ls.mounts[id]
	ls.mountL.Unlock()
	if mount == nil {
	...
	return mount.getOriginalReference()
}
