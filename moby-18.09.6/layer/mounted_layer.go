...
func (ml *mountedLayer) MountID() string {
	return ml.mountID
}
...
func (ml *mountedLayer) getOriginalReference() (RWLayer, error) {
	if len(ml.references) > 1 {
		return nil, errors.New("layer has more than one referenced")
	}
	for rwlayer := range ml.references {
		return rwlayer, nil
	}
	return nil, errors.New("layer has no referenced")
}
