...
func (b *Builder) commit(dispatchState *dispatchState, comment string, cfgs chan backend.CommitConfig) error {
	if b.disableCommit {
		return nil
	}
	...
	return b.commitContainer(dispatchState, id, runConfigWithCommentCmd, cfgs)
}

func (b *Builder) commitContainer(dispatchState *dispatchState, id string, containerConfig *container.Config, cfgs chan backend.CommitConfig) error {
	if b.disableCommit {
		return nil
	}
	...
	if cfgs != nil {
		confusedCfg := backend.CommitConfig{
			Author: dispatchState.maintainer,
			// TODO: this copy should be done by Commit()
			Config:          copyRunConfig(dispatchState.runConfig),
			ContainerConfig: containerConfig,
		...
		cfgs <- confusedCfg
		imageID, err = b.docker.CommitBuildStep(commitCfg, true)
	} else {
		imageID, err = b.docker.CommitBuildStep(commitCfg, false)
	}
	dispatchState.imageID = string(imageID)
	return err
}

func (b *Builder) exportConfusedImage(c backend.CommitConfig) (image.ID, error) {
	exportedImage, roLayer, err := b.docker.CommitAddOrCopy(c)
	if err != nil {
		return "", err
	}
	b.imageSources.Add(newImageMount(exportedImage, roLayer))
	return exportedImage.ID(), nil
}

func (b *Builder) exportImage(state *dispatchState, layer builder.RWLayer, parent builder.Image, runConfig *container.Config, confused bool) error {
	var newLayer builder.ROLayer
	var err error
	if confused {
		newLayer, err = layer.ConfusedCommit()
	} else {
...
func (b *Builder) performCopy(req dispatchRequest, inst copyInstruction, cfgs chan backend.CommitConfig) error {
	state := req.state
	...
	if cfgs != nil {
		parent := imageMount.Image()
		parentImage, ok := parent.(*image.Image)
		if !ok {
			return errors.Errorf("unexpected image type")
		}
		cfg := backend.CommitConfig{
			Author:              state.maintainer,
			Config:              copyRunConfig(state.runConfig),
			ContainerConfig:     runConfigWithCommentCmd,
			ContainerID:         rwLayer.Name(),
			ContainerMountLabel: "### PerformCopy Action ###",
			ContainerOS:         parentImage.OS,
			ParentImageID:       state.imageID,
		}
		cfgs <- cfg
	}
	return b.exportImage(state, rwLayer, imageMount.Image(), runConfigWithCommentCmd, cfgs != nil)
}
